create or replace FUNCTION HCD_OBTER_DADOS_LOTE_CP (nr_seq_lote_p number, cd_material_p number, nr_seq_superior_p number, ie_posicao number, ie_tipo_lote varchar2, ie_opcao varchar2)
RETURN VARCHAR2

/*<DS_SCRIPT>
 DESCRIÇÃO...: FUNÇÃO QUE RETORNA OS DADOS REFERENTE AO HORÁRIO E DOSAGEM DOS DILUENTES DO MEDICAMENTO DOS LOTES NORMAIS E AGRUPADOS
                ...
                THEN
                    IF(ie_opcao = 'H') 
                        THEN
                            ...
                            where (nr_seq_lote = nr_seq_lote_p)
                            and (cd_material = cd_material_p)
                            and (( nr_seq_superior = nr_seq_superior_p) OR (nr_seq_superior_p is null))
                            and (to_char(dt_horario,'hh24:mi') between '00:00' and '01:59');
                    ELSIF (ie_opcao = 'D')
                        THEN
                            select distinct
